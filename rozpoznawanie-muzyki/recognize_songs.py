...
def group_matches(matchesx):
    ...
    for matches_tuple in matchesx:
        ...
        # print("matches_tuple: ", matches_tuple)
        if offset not in offset_dict:
            offset_dict[offset] = {}
            # print("offset_dict[offset]: ", offset_dict[offset])
        if sid not in offset_dict[offset]:
            offset_dict[offset][sid] = 0
            # print("offset_dict[offset][sid]: ", offset_dict[offset][sid])
        offset_dict[offset][sid] += 1
        # print("(offset_dict[offset][sid]: ", offset_dict[offset][sid])
        if offset_dict[offset][sid] > confidence:
            max_offset = offset
            # print("max_offset: ", max_offset)
            confidence = offset_dict[offset][sid]
    ...
    return {
        ...
        "CONFIDENCE_PERCENT": confidence / len(matchesx) * 100
    }
...
if __name__ == '__main__':
    ...
    for channel_number, channel_samples in enumerate(split_data):
        #print(len(channel_samples), channel_samples[:10])
    ...
    if len(hash_matches) > 0:
        ...
        print((' => utwór: %s (id=%d)\n' + '    zakres: 0-%d (0-%0.2f sekund)\n' +
              ...
              (song['SONG_NAME'], song['SONG_ID'],
               ...
               song['CONFIDENCE'], len(hash_matches), song['CONFIDENCE_PERCENT']))
    else:
